...
function jieqi_obook_obookvars($obook)
{
	...
	$ret['sort']         = isset($jieqiSort['obook'][$ret['sortid']]['caption']) ? $jieqiSort['obook'][$ret['sortid']]['caption'] : '';
	$ret['publisher']   = isset($jieqiPublisher[$ret['publishid']]['name']) ? $jieqiPublisher[$ret['publishid']]['name'] : '';
	/*
			$authorid =$ret['authorid'];
			$result = mysql_query("select banktype,bankcard,bankuser from jieqi_article_author where uid = $authorid");
			$row = mysql_fetch_array($result);
			$ret['banktype'] = $row['banktype'];
			$ret['bankcard'] = $row['bankcard'];
			$ret['bankuser'] = $row['bankuser'];
		*/
