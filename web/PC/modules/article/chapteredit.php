...
//2015.8.27 24小时后编辑章节权限 开始
$edittime = jieqi_checkpower($jieqiPower['article']['edittime'], $jieqiUsersStatus, $jieqiUsersGroup, true);
$lastupdate = date("Y-m-d H:i:s", $chapter->getVar("lastupdate"));
$update = date("Y-m-d H:i:s", strtotime("+1 day", strtotime("$lastupdate")));
$timeout = strtotime($update);
if (!$edittime){
if (JIEQI_NOW_TIME > $timeout){
	jieqi_printfail(sprintf($jieqiLang["article"]["noper_edittime_chapter"], $typename));
  }
}
//24小时后编辑章节权限 结束
