...
{
	$sql = "SELECT * FROM ".jieqi_dbprefix("article_upload")." WHERE id=".intval($_POST['cover'])." AND type='cover' AND sign='" . $sign . "' AND status=0";
	
	$query = $query_handler->db->query($sql);
...
$query = $query_handler->db->query("SELECT * FROM ".jieqi_dbprefix("article_upload")." WHERE id='".intval($_POST['ebook'])."' AND type=0 AND sign='$sign' AND status='0'");
