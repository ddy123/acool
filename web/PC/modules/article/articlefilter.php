...
$tmpary = explode(' ', $jieqiFilter['article']['order'][$filterary['order']]['order']);
$orderfield = $tmpary[0];
$jieqiTpl->assign('orderfield', $tmpary[0]);
...
//include_once JIEQI_ROOT_PATH . '/configs/article/tag.php';
//if (isset($jieqiTag['article'])) {
if (isset($_GET['tag'])) {
	if ($jieqiTag['article'][$_GET['tag']]) {
		$filterary['tag']=$_GET['tag'];
		$tag = $jieqiTag['article'][$_GET['tag']]['caption'];
		$filtersql .= " AND keywords LIKE '%" . $tag . "%'";
	}
	else {
		unset($_GET['tag']);
	}
...
$tagrows = array();
$tagrows[0]['name'] = $jieqiLang['article']['article_filter_unlimit'];
$varary = $_GET;
...
$tagrows[0]['url'] = jieqi_geturl('article', 'articlefilter', 1, $varary);
$tagrows[0]['selected'] = isset($filterary['tag']) ? 0 : 1;
$i = 1;

foreach ($jieqiTag['article'] as $k => $v) {
	$tagrows[$i]['name'] = $v['caption'];
	$varary['tag'] = $k;
	if(empty($_GET['tag'])){
		$tagids= 0 ;
	}
	...
	$tagrows[$i]['id'] =$tagids;
	$tagrows[$i]['url'] = jieqi_geturl('article', 'articlefilter', 1, $varary);
	$tagrows[$i]['selected'] = isset($filterary['tag']) && ($filterary['tag'] == $k) ? 1 : 0;
	$i++;
}
...
//}
/*
if (isset($jieqiFilter['article']['tag'])) {
if (isset($_GET['tag'])) {
	if (isset($jieqiFilter['article']['tag'][$_GET['tag']])) {
		$filterary['tag']=$_GET['tag'];
		$tag = $jieqiFilter['article']['tag'][$_GET['tag']]['caption'];
		$filtersql .= " AND keywords LIKE '%" . $tag . "%'";
	}
	else {
		unset($_GET['tag']);
...
foreach ($jieqiFilter['article']['tag'] as $k => $v) {
	$tagrows[$i]['name'] = $v['caption'];
	$varary['tag'] = $k;
	$tagrows[$i]['url'] = jieqi_geturl('article', 'articlefilter', 1, $varary);
	$tagrows[$i]['selected'] = isset($filterary['tag']) && ($filterary['tag'] == $k) ? 1 : 0;
...
if (!$content_used_cache) {
	...
	while ($v = $article_handler->getObject()) {
		...
		$articlerows[$k]['ordervalue'] = isset($articlerows[$k][$orderfield]) ? $articlerows[$k][$orderfield] : '';
...
$rgroup = $_GET['rgroup'];
//$jieqiTpl->assign('group', $group);
$jieqiTpl->assign('rgroup', $rgroup);
