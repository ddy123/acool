...
$articleedit = jieqi_checkpower($jieqiPower["article"]["articleedit"], $jieqiUsersStatus, $jieqiUsersGroup, true);

if (!$articleedit) {
	jieqi_printfail($jieqiLang["article"]["noper_edit_articleedit"]);
}
	...
	if (empty($errtext))
	{
		...
		if ($allowmodify) {
			...
			if (isset($_POST["issign"])) {
				...
				if (0 < $_POST["issign"]) {
					...
					if (($_POST["issign"] == 0) && (0 < intval($article->getVar("signtime", "n")))) {
		...
		if (!$article_handler->insert($article)) {
			...
			if (!empty($_FILES["articlespic"]["name"]))
			{
				...
				$sToImgFilePath = $package->getDir("imagedir") . "/" . $id . "s" . $simage_postfix;
				
				jieqi_delfile($sToImgFilePath);
				
				$tmpFLag = jieqi_copyfile($_FILES["articlespic"]["tmp_name"], $sToImgFilePath, 777, true);
			}
			...
			if (!empty($_FILES["articlelpic"]["name"]))
			{
				...
				$lToImgFilePath = $package->getDir("imagedir") . "/" . $id . "l" . $limage_postfix;
				
				jieqi_delfile($lToImgFilePath);
				
				jieqi_copyfile($_FILES["articlelpic"]["tmp_name"], $lToImgFilePath, 777, true);
			}
