...
function jieqi_get_block($blockconfig, $retflag = 0)
{
	...
	if ((($jieqiUsersStatus == JIEQI_GROUP_GUEST) &&
			(0 < ($blockconfig["publish"] & 1))) ||
		(($jieqiUsersStatus != JIEQI_GROUP_GUEST) &&
			(0 < ($blockconfig["publish"] & 2))))
	{
		$jieqi_blockside = "";
		...
		if (($blockconfig["contenttype"] != JIEQI_CONTENT_PHP) &&
			empty($blockconfig["hasvars"]))
		{
			if (0 < $blockconfig["custom"])
			{
...
if (defined("JIEQI_MOBILE_LOCATION") && (0 < strlen(trim(JIEQI_MOBILE_LOCATION))) && defined("JIEQI_LOCAL_URL") && (strpos(JIEQI_LOCAL_URL, JIEQI_MOBILE_LOCATION) === false) && jieqi_ismobile()) {
	header("Location: " . jieqi_headstr(JIEQI_MOBILE_LOCATION));
	exit();
}
...
//burn添加 2016-12-15，首页的作品分类分类
include_once('/configs/article/sort.php');
...
$jieqiTpl->assign("artileSort", $jieqiSort['article']);
$jieqiTpl->assign("artileSortCount", count($jieqiSort['article']));
