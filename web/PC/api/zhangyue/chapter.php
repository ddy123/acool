...
$sign=strtolower(md5($client_id.$client_secret.$_GET['book_id'].$_GET['chapter_id']));
...
if(!empty($_GET['book_id']) && !empty($_GET['chapter_id'])){
	//输出某一页小说列表
	...
	$query = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  zhangyue = 1 AND articleid= ".$_GET['book_id'] );
	$upinfo = $query_handler->getobject($query);
if($upinfo){	
    $chapter = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_chapter') . " WHERE  chapterid = ".$_GET['chapter_id']." AND articleid= ".$_GET['book_id'] );
...
//$txtcontent.="</p>\n";  
	...
	$ocontent = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('obook_ocontent') . " WHERE  ochapterid = ".$_GET['chapter_id']);
...
<chapter>  
 <id>{$item['cid']}</id>           
 <title><![CDATA[{$item['chaptername']}]]></title>                
 <content><![CDATA[{$item['txtcontent']}]]></content>      
 <create_time >{$item['lastupdate']}</create_time >
<is_free>{$item['isvip']}</is_free> 
