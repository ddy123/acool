...
if ($isconnect) {
	...
	if (empty($errtext)) {
		...
		if (is_array($config_array) && (0 < count($config_array))) {
			$i = 0;

			for (; $i < count($config_array); $i++) {
	...
	if ($_POST['system_user'] && $_POST['system_pass'] && $_POST['system_pass_confirm']) {
		...
		else if (!preg_match('/^[_a-z0-9-]+(\\.[_a-z0-9-]+)*@[a-z0-9-]+([\\.][a-z0-9-]+)+$/i', $_POST['system_email'])) {
			$errtext .= $jieqiLang[JIEQI_MODULE_NAME]['error_email_format'] . '<br />';
		}
	}
}
else {
