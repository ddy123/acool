...
if (@is_array($_COOKIE['jieqi_step'])) {
	if (!@in_array(CURRENT_STEP, $_COOKIE['jieqi_step']) && !@in_array(0, $_COOKIE['jieqi_step'])) {
...
while ($mod_dir = $jieqiDir->read()) {
	if (is_dir($sql_dir . '/' . $mod_dir) && ($mod_dir !== '.') && ($mod_dir !== '..')) {
		if (file_exists($sql_dir . '/' . $mod_dir . '/modinfo.php')) {
			...
			if (isset($jieqiModinfo[$mod_dir])) {
				$mod_array[$i] = $jieqiModinfo[$mod_dir];
			}
		}
		else {
		...
		if (in_array(strtolower($mod_dir), $inmodules)) {
			$mod_array[$i]['status'] = 3;
		}
		else if (strtolower($mod_dir) == 'system') {
			$mod_array[$i]['status'] = 2;
		}
		else if (file_exists($sql_dir . '/' . $mod_dir . '/struct.sql')) {
			$mod_array[$i]['status'] = 1;
		}
		else {
			$mod_array[$i]['status'] = 0;
		}
	}
