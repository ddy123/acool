<?php
$siteurl = 'http://www.mianfeidushu.com'; //请设置跳转到的网站
...
if(isset($_SERVER['REQUEST_URI'])) $pageurl = $_SERVER['REQUEST_URI'];
else{
	$pageurl = isset($_SERVER['PHP_SELF']) ? $_SERVER['PHP_SELF'] : $_SERVER['SCRIPT_NAME'];
	if(isset($_SERVER['QUERY_STRING']) && strlen($_SERVER['QUERY_STRING']) > 0) $pageurl .= '?'.$_SERVER['QUERY_STRING'];
	elseif(isset($_SERVER['argv'][0]) && strlen($_SERVER['argv'][0]) > 0) $pageurl .= '?'.$_SERVER['argv'][0];
}
