...
class BlockArticleSales extends JieqiBlock
{
	public $module = "article";
	public $template = "block_sales.html";
	public $exevars = array("field" => "day", "listnum" => 10, "asc" => 0);

	public function BlockArticleSales(&$vars)
	{
		global $jieqiTpl;
		global $jieqiTop;
	...
	public function setContent($isreturn = false)
	{
		...
		$sql = "SELECT *,sum(egold) as egoldnum FROM " . jieqi_dbprefix("article_actlog") . " WHERE $slimit GROUP BY articleid ORDER BY sum(egold) $order LIMIT $limitstart,$listpnum";
		$salesrows = array();
		$query->execute($sql);
		$k = 0;

		while ($row = $query->getRow()) {
			$salesrows[$k] = jieqi_funtoarray("jieqi_htmlstr", $row);
			
			$k++;
		}

		$jieqiTpl->assign_by_ref("salesrows", $salesrows);
	}
}
