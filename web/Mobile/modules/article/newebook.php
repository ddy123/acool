...
if (empty($_POST['ebook'])) jieqi_printfail("电子书上传失败1！");
...
$upinfo = $query_handler->getobject($query);
//var_dump($upinfo);
if (empty($upinfo)) jieqi_printfail("电子书上传失败2！");
...
if (empty($readlist)) $errtext .= "电子书解析失败！<br />";
include_once JIEQI_ROOT_PATH . '/lib/text/textfunction.php';
...
foreach ($readlist as $key => $val) {
	...
	if (!$chapter_handler->insert($newChapter)) continue;
