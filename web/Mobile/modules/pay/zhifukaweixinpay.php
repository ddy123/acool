...
jieqi_loadlang("pay", "pay");
		...
		$item['customerid'] = "155022"; //商户号
		$item['key'] = "2364400b4f34feef9cb9b11b955e4d71";//秘钥
		$item['sdcustomno'] = $mch_vno;//订单在商户系统中的流水号
		$item['orderAmount'] = $_POST['egold'];//订单支付金额；单位:分(人民币)

		$item['cardno'] = "32";//固定值32为（微信）  36为（手机QQ）
		$item['noticeurl'] = "".JIEQI_URL."/modules/pay/weixinpayreturn.php";
		...
		$gourl='http://www.zhifuka.net/gateway/weixin/wap-weixinpay.asp?customerid='.$customerid.'&sdcustomno='.$sdcustomno.'&orderAmount='.$orderAmount.'&cardno='.$cardno.'&noticeurl='.$noticeurl.'&backurl='.$backurl .'&sign='.$sign.'&mark='.$mark;
