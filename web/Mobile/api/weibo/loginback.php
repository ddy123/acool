...
if (isset($_GET['code']) && ($_SESSION['jieqiUserApi'][$apiName]['state'] == $_GET['state']))
{
	...
	$ret = OpenSDK_Sina_Weibo2::getAccessToken('code', array('code' => $_GET['code'], 'redirect_uri' => 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF']));

	if (isset($ret['error']))
	{
		jieqi_printfail(sprintf($jieqiLang['system']['weibo_access_token_failure'], $ret['error'], $ret['error_description']));
	}
}
else
{
	jieqi_printfail($jieqiLang['system']['weibo_error_callback_params']);
}

$_SESSION['jieqiUserApi'][$apiName]['access_token'] = OpenSDK_Sina_Weibo2::getParam(OpenSDK_Sina_Weibo2::ACCESS_TOKEN);
$_SESSION['jieqiUserApi'][$apiName]['refresh_token'] = OpenSDK_Sina_Weibo2::getParam(OpenSDK_Sina_Weibo2::REFRESH_TOKEN);
$_SESSION['jieqiUserApi'][$apiName]['expire_in'] = JIEQI_NOW_TIME + OpenSDK_Sina_Weibo2::getParam(OpenSDK_Sina_Weibo2::EXPIRES_IN);
$_SESSION['jieqiUserApi'][$apiName]['openid'] = OpenSDK_Sina_Weibo2::getParam(OpenSDK_Sina_Weibo2::OAUTH_USER_ID);
$_SESSION['jieqiUserApi'][$apiName]['openkey'] = '';

if (empty($_SESSION['jieqiUserApi'][$apiName]['openid']))
{
	jieqi_printfail($jieqiLang['system']['weibo_error_callback_getvar']);
}
