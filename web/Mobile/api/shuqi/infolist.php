...
//百度站内搜索接口，输出本站小说更新信息
// baidu_sitemap.php 全部小说分页索引
// baidu_sitemap.php?page=1 全部小说列表第1页
// baidu_sitemap.php?update=2 最近2小时内有更新的小说分页索引
// baidu_sitemap.php?page=1&update=2 最近2小时内有更新的小说列表第1页
...
header("Content-type: text/xml");
	...
	$query = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  shuqi = 1 AND articleid= ".$_GET['id'] );
	$upinfo = $query_handler->getobject($query);
if($upinfo){
	echo '<?xml version="1.0" encoding="UTF-8"?>
 <chapterinfo>';
echo '<bookid><![CDATA['.$_GET['id'].']]></bookid>';
...
echo "<volume><volumeid><![CDATA[{$k}]]></volumeid>
	<volname><![CDATA[正文]]></volname><chapters><chapter>
<chapterid><![CDATA[{$chapterrows['chapterid']}]]></chapterid>
<chaptername><![CDATA[{$chapterrows['chaptername']}]]></chaptername>
</chapter>";
}elseif($k == 1  && $chapterrows['chaptertype'] == 1){
	echo "<volume><volumeid><![CDATA[{$chapterrows['chapterid']}]]></volumeid>
	<volname><![CDATA[{$chapterrows['chaptername']}]]></volname><chapters>
";

}elseif($k != 1 && $chapterrows['chaptertype'] == 1){
	echo "</chapters></volume><volume><volumeid><![CDATA[{$chapterrows['chapterid']}]]></volumeid>
	<volname><![CDATA[{$chapterrows['chaptername']}]]></volname><chapters>
";
}
else{
...
<chaptername><![CDATA[{$chapterrows['chaptername']}]]></chaptername>
	...
	echo '</chapters></volume></chapterinfo>';
}else{
echo '<?xml version="1.0" encoding="UTF-8"?>
<error/>';
}
