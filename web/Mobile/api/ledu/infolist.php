...
header("Content-type: text/xml");
	...
	$query = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  ledu = 1 AND articleid= ".$_GET['id'] );
...
while($chapter = $chapter_handler->getObject()){
	...
	$chapterrows['url'] = $url . "/api/ledu/chapter.php?id=" . $chapterrows['articleid'] . "&amp;cid=" . $chapterrows['chapterid'];
	$k++;
