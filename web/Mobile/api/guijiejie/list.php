...
$sign=strtolower(md5($client_id.$client_secret));
...
header("Content-type: text/xml");
	...
	$sql = "SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  guijiejie = 1  ORDER BY lastupdate DESC";
	$query->execute($sql);

	echo '<?xml version="1.0" encoding="UTF-8"?>
<document><webName><![CDATA['.JIEQI_SITE_NAME.']]></webName>
 <webSite><![CDATA['.JIEQI_URL.']]></webSite>';
	...
	while($row = $query->getRow()){
		...
		$item['updateStatus'] = $row['fullflag'] == 1 ? 1 : 0;
  ...
  <size><![CDATA[{$item['wordCount']}]]></size>
  <tag><![CDATA[{$item['keywords']}]]></tag>
  <fullflag><![CDATA[{$item['updateStatus']}]]></fullflag>
  <comment><![CDATA[{$item['description']}]]></comment> 
  <imageLink><![CDATA[".JIEQI_URL."]]></imageLink>
