...
$sign=strtolower(md5($client_id.$client_secret.$_GET['book_id']));
...
if(!empty($_GET['book_id'])){
	...
	$query = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  guijiejie = 1 AND articleid= ".$_GET['book_id'] );
...
if(empty($_REQUEST['book_id'])) jieqi_printfail(LANG_ERROR_PARAMETER);
jieqi_loadlang('manage', JIEQI_MODULE_NAME);
include_once($jieqiModules['article']['path'].'/class/article.php');
$article_handler =& JieqiArticleHandler::getInstance('JieqiArticleHandler');
$article=$article_handler->get($_REQUEST['book_id']);
//包含区块参数
...
$criteria=new CriteriaCompo(new Criteria('articleid', $_REQUEST['book_id'], '='));
...
while($chapter = $chapter_handler->getObject()){
	...
	$chapterrows['lastupdate'] = date('Y-m-d h:i:s',$chapter->getVar('lastupdate'));
	if($chapter->getVar('isvip') > 0){
		$chapterrows['isvip'] = 1;
	}else{
		$chapterrows['isvip'] = 0;
	}
	$chapterrows['articleid'] = $chapter->getVar('articleid');
	if($chapterrows['chaptertype'] == 0){
	$chapterrows['url'] = JIEQI_URL . "/api/guijiejie/chapter.php?book_id=" . $chapterrows['articleid'] . "&chapter_id=" . $chapterrows['chapterid'];
	}
	$k++;


echo "<item>
<cid><![CDATA[{$chapterrows['chapterid']}]]></cid>
<isvip><![CDATA[{$chapterrows['isvip']}]]></isvip>
<name><![CDATA[{$chapterrows['chaptername']}]]></name>
<chaptertype><![CDATA[{$chapterrows['chaptertype']}]]></chaptertype>
<path><![CDATA[{$chapterrows['url']}]]></path>
</item>";	
