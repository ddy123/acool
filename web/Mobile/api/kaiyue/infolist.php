...
header("Content-type: text/xml");
	...
	$query = $query_handler->db->query("SELECT * FROM " . jieqi_dbprefix('article_article') . " WHERE  kaiyue = 1 AND articleid= ".$_GET['id'] );
...
while($chapter = $chapter_handler->getObject()){
	...
	$chapterrows['url'] = $url . "/api/kaiyue/chapter.php?id=" . $chapterrows['articleid'] . "&amp;cid=" . $chapterrows['chapterid'];
	$k++;
...
echo "
<chapter>
<order>{$chapterrows['chapterorder']}</order>
<url>{$chapterrows['url']}</url>
