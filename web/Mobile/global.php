...
function jieqi_checkpower2($powerset = array(), $ustatus = '0', $ugroup = '0', $isreturn = false, $isadmin = false)
{
	if (empty($_POST)) {
		$local_domain_url = (empty($_SERVER['HTTP_HOST']) ? '' : 'http://' . $_SERVER['HTTP_HOST']);
		$jumpurl = $local_domain_url . jieqi_addurlvars(array());
	...
	if ($ustatus == JIEQI_GROUP_ADMIN) {
		if ($isadmin && empty($_SESSION['jieqiAdminLogin'])) {
			if ($isreturn) {
				...
				header('Location: ' . jieqi_headstr(JIEQI_LOCAL_URL . '/tool/login.php?jumpurl=' . urlencode($jumpurl)));
				exit();
			}
		}
		else {
		...
		if (is_array($powerset['groups']) && (in_array($ugroup, $powerset['groups'], false) || in_array('0', $powerset['groups'], false))) {
			...
			if (empty($_REQUEST['ajax_request'])) {
				if ($isadmin) {
					header('Location: ' . jieqi_headstr(JIEQI_USER_URL . '/tool/login.php?jumpurl=' . urlencode($jumpurl)));
				}
				else {
					header('Location: ' . jieqi_headstr(JIEQI_USER_URL . '/login.php?jumpurl=' . urlencode($jumpurl)));
				}
...
$_USER['source'] =intval($_SESSION['jieqiUserSource']);
...
if(getUsersStatus() != JIEQI_GROUP_ADMIN) return false;
else return true;
}
...
$a_tmp = array();
foreach($ip_arrays as $v){$a_tmp[]=trim($v);}
if(in_array($ip,$a_tmp)) jieqi_printfail('���ѱ�ϵͳ���Ʋ�����');
}
...
$_SESSION['submitcount'] = $_SESSION['submitcount'] +1;
	...
	$newlog->setVar('toid', 0);
	$newlog->setVar('toname', '');
	$newlog->setVar('reason', '�������ݻ���');
	$newlog->setVar('chginfo', $ip."��һ����������������������� ".($_SESSION['submitcount'])." �α�����!�ж�Ϊ�����û������û��Ѿ���".($_SESSION['jieqiUserId'] ?'��Ϊ�ο�': '��ֹ�ύ��')."��");
...
swritefile($deniedip,$ip."\r\n","a+");
jieqi_printfail('ϵͳ��⵽�Ƿ�������Դ�����IP:'.$ip.'�����Ѿ���¼���������뿪��');
}
}
...
return (getTime('month')-$premonthday*86400);
...
function toXml($inputArray)
{
	...
	{
		jieqi_printfail(iconv("UTF-8","GB2312","�������쳣"));
	}
...
function postXmlCurl($xml, $url, $second = 30)
{
	...
	else
	{
		...
		jieqi_printfail(iconv("UTF-8","GB2312","curl����������:$error"));
	}
}
...
function fromXml($xml)
{
	if(!$xml)
	{
		jieqi_printfail(iconv("UTF-8","GB2312","xml�����쳣��"));
	}
...
function checkSign($inputArray,$key)
{
	...
	{
		jieqi_printfail(iconv("UTF-8","GB2312","ǩ����Ϣ����"));
	}
	...
	jieqi_printfail(iconv("UTF-8","GB2312","ǩ������"));
}
...
if((!empty($_POST) || !empty($_REQUEST['act']))&&(!empty($_POST) || !empty($_REQUEST['action']))&& basename($_SERVER['PHP_SELF'])!='login.php') jieqi_printfail('�Բ��𣬲����ʺŲ���������޸Ĺ������ݣ�');
}
