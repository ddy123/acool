...
case "new":
	...
	$_POST["maxgold"] = intval($_POST["maxgold"]);
	...
	if (empty($errtext)) {
		...
		$vips->setVar("maxgold", $_POST["maxgold"]);
...
case "update":
	if (!empty($_REQUEST["id"]) && !empty($_POST["caption"])) {
		...
		if (is_object($vips)) {
			...
			if (!is_numeric($_POST["maxgold"])) {
				$errtext .= $jieqiLang["system"]["need_maxgold_num"] . "<br />";
			}
...
case "edit":
	if (!empty($_REQUEST["id"])) {
		...
		if (is_object($vips)) {
			...
			$vips_form->addElement(new JieqiFormTextArea($jieqiLang["system"]["table_vips_maxgold"], "maxgold", $vips->getVar("maxgold", "e"), 5, 50));
...
while ($v = $vips_handler->getObject()) {
	...
	$vipary[$v->getVar("vipid")] = array("caption" => $nameary[0], "minegold" => $v->getVar("minegold"), "maxgold" => $v->getVar("maxgold"), "extraegold" => $v->getVar("extraegold"), "extradiv" => $v->getVar("extradiv"));
	$vips[$i]["vipid"] = $v->getVar("vipid");
	$vips[$i]["caption"] = implode("<br />", $nameary);
	$vips[$i]["minegold"] = $v->getVar("minegold");
	$vips[$i]["maxgold"] = $v->getVar("maxgold");
...
$vips_form->addElement(new JieqiFormText($jieqiLang["system"]["table_vips_maxgold"], "maxgold", 30, 50, ""), true);
