<?php
echo '<form name="batchrepack" id="batchrepack" action="'.$this->_tpl_vars['url_batchrepack'].'" target="_blank" method="post" onsubmit="return confirm(\'确实要开始重新生成么？\');">
<table class="grid" width="100%" align="center">
<caption>小说批量生成</caption>
<tr>
  <td class="tdl" width="25%">限制条件：</td>
  <td class="tdr" width="75%">
  <input name="act" type="radio" value="packwithid" checked="checked"> 小说ID从 <input type="text" class="text" name="fromid" id="fromid" size="10" maxlength="15" value="'.$this->_tpl_vars['minaid'].'" /> 到 <input type="text" class="text" name="toid" id="toid" size="10" maxlength="15" value="'.$this->_tpl_vars['maxaid'].'" /> <span class="hot">当前数据库小说ID为：'.$this->_tpl_vars['minaid'].' - '.$this->_tpl_vars['maxaid'].'</span>
  <hr />
  <input name="act" type="radio" value="packwithtime"> 更新时间从 <input type="text" class="text" name="starttime" id="starttime" size="20" maxlength="20" value="" /> 到 <input type="text" class="text" name="stoptime" id="stoptime" size="20" maxlength="20" value="" /> <span class="hot">时间格式举例：2005-01-23 23:06:30</span>
  </td>
</tr>
<tr>
  <td class="tdl">分类：</td>
...
<tr>
  <td class="tdl">&nbsp;'.$this->_tpl_vars['jieqi_token_input'].'</td>
  <td class="tdr"><input type="submit" class="button" name="submit" value="开始生成" /></td>
</tr>
</table>
</form>';
?>