 ...
 * @return mixed
 */
function get_user_info($user_id_or_name,$oauth)
{
	$map['uname']     = $user_id_or_name;
	
	$map['logintype'] = $oauth;
	
	$user = M('distribution_system_users')->where($map)->find();
...
function goods_thum_images($goods_id,$width,$height)
{
	...
	elseif(file_exists($path.$goods_thumb_name.'.jpeg'))
	{
		return '/'.$path.$goods_thumb_name.'.jpeg'; 
	}
	elseif(file_exists($path.$goods_thumb_name.'.gif'))
	{
		return '/'.$path.$goods_thumb_name.'.gif'; 
	}
	elseif(file_exists($path.$goods_thumb_name.'.png'))
	{
...
function refresh_stock($goods_id)
{
	...
	if($count == 0)
	{
		return false; // 没有使用规格方式 没必要更改总库存
	}
...
{
// 		require_once THINK_PATH.'Library/Vendor/phpmailer/PHPMailerAutoload.php';//邮件服务
// 	    $mail   = new PHPMailer;

	//burn修改，2016-07-29
	require_once THINK_PATH.'Library/Vendor/phpmailer/class.phpmailer.php';//邮件服务
	...
	$body.=' <style> .footer-info {
        ...
        .footer-info h3 {
	     ...
	     <div class="footer-info">
          <h3>趣读商城服务管理中心</h3>
          <p>系统发信，请勿回复。你的验证码是：'.$content.'</p>
          <p>趣读商城服务热线：0371-60999308</p>
        </div>
        <br>';
...
function calculate_price($user_id = 0,$order_goods,$shipping_code = '',$shipping_price = 0,$province = 0,$city = 0,$district = 0,$pay_points = 0,$user_money = 0,$coupon_id = 0,$couponCode = '')
{
	...
	$goods_arr    = M('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price'); //商品id和重量对应的键值对
	    
	foreach($order_goods as $key => $val)
	{
		// 如果传递过来的商品列表没有定义会员价
		if(!array_key_exists('member_goods_price',$val))//检查$val中是否存在'member_goods_price'的键值
		{
			$user['discount'] = $user['discount'] ? $user['discount'] : 1; //会员折扣不能为0，默认为1
	...
	$pay_points   = ($pay_points / tpCache('shopping.point_rate')); //积分支付100积分等于1块钱                              
	$pay_points   = ($pay_points > $order_amount) ? $order_amount : $pay_points; // 假设应付1块钱而用户输入了 200 积分 2块钱,那么就让 $pay_points = 1 块钱等同于强制让用户输入1块钱               
...
function get_goods_category_tree()
{
	...
	foreach ($cat_list as $val)
	{
		if($val['level'] == 2)
		{
			$arr[$val['parent_id']][] = $val;//2级商品分类
		}
		elseif($val['level'] == 3)
		{
			$crr[$val['parent_id']][] = $val;//3级商品分类
		}
		elseif($val['level'] == 1)
		{
			$tree[] = $val;//1级商品分类
